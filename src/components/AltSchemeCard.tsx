...
interface AltSchemeCardProps {
  ...
  /** Full-width, for a vertical list (SPEC 8.5 /discover). Default: fixed-width, for
   * the horizontal-scroll "Other schemes for you" row (SPEC 8.4 item 7). */
  fullWidth?: boolean;
}

// One alternative scheme: name, verdict chip, benefit, 2 reasons, "Open". SPEC.md Section 8.4 item 7.
export function AltSchemeCard({ suggestion, fullWidth = false }: AltSchemeCardProps) {
  const t = useT();
  ...
  return (
    <div
      className={`flex flex-col gap-2 rounded-2xl border border-ink/10 bg-white p-4 ${fullWidth ? 'w-full' : 'w-72 flex-none'}`}
    >
      <span
